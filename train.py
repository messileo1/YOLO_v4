...
if __name__ == "__main__":
    ...
    #
    #   model_path使用的是整个模型的权重。
    #   如果想要让模型从主干的预训练权值开始训练，则首先设置model_path = ''，网络不加载整个权重，然后设置下面的pretrain = True，此时仅加载主干。
    ...
    #                   如果设置了model_path，则主干的权值无需加载，pretrained的值无意义，因为不管加不加载都会被覆盖掉。
    ...
    #                   但本项目没用到标签平滑，所以设置了也没用。如果要用则需要对代码进行调整修改，具体见dataloader.py文件。
    #   余弦退火算法的参数放到下面的lr_decay_type中设置。lr随epoch具体怎么变的看get_lr_table.py。
    #------------------------------------------------------#
    mosaic              = True
    label_smoothing     = 0
    ...
    #------------------------------------------------------------------#
    #   冻结阶段训练参数
    #   此时模型的主干被冻结了，特征提取网络不发生改变,因此batch_size和学习率可以大一点。
    #   占用的显存较小，仅对网络进行微调
    ...
    #------------------------------------------------------------------#
    num_workers         = 4
    '''重要：windows用户如果要使用多核多线程必须把训练放在if __name__ == '__main__':下才不会报错！'''
    #------------------------------------------------------#
    ...
    #   创建yolo模型
    #------------------------------------------------------#
    model = YoloBody(anchors_mask, num_classes, pretrained = pretrained) #pretrained用于判断创建yolov4网络时是否加载主干特征网络的预训练权重。
    if not pretrained: #如果没加载主干网络的特征那么初始化网络权重
        weights_init(model)
    ...
    yolo_loss    = YOLOLoss(anchors, num_classes, input_shape, Cuda, anchors_mask, label_smoothing, focal_loss, alpha, gamma) #定义损失函数
    loss_history = LossHistory(save_dir, model, input_shape=input_shape) #定义损失的保存路径
    ...
    if True:
        ...
        #---------------------------------------#
        #   根据optimizer_type选择优化器，看set_optimizer_test.py文件。
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        ...
        # {}创建了一个字典，通过key（adam/sgd）获取里面的value。

        '''add_param_group方法接受一个param_group参数，该参数是一个字典。
        用此方法可以建立一个渐进网络，使得不同的层，或者不同的参数有不同的学习率，权重衰减等。
        如果不设置学习率，那么就采用默认值。要设置的话，参照下面这种方式以字典的方式设置。'''
        optimizer.add_param_group({"params": pg1, "weight_decay": weight_decay}) #对于weight采用权重衰减（正则化）
        optimizer.add_param_group({"params": pg2})

        #---------------------------------------#
        #   获得学习率下降的公式，这儿根据传入的参数（学习率下降方式，最大最小学习率，总的训练轮数），
        #   返回一个函数lr_scheduler_func，需要传入当前的轮数就会返回根据函数确定的学习率
        #---------------------------------------#
        lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, UnFreeze_Epoch)
        
        #---------------------------------------#
        #   判断每一个世代的长度（iteration）
        #---------------------------------------#
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                for param in model.backbone.parameters():
                    param.requires_grad = True #解冻主干网络
            ...
            '''当处于冻结阶段，epoch<=Freeze_Epoch，从此处开始执行'''
            gen.dataset.epoch_now       = epoch #获取当前epoch，判断是否使用mosaic数据增强
            #通过这种方法对dataloader进行动态设置，比如gen.num_workers = 2
            gen_val.dataset.epoch_now   = epoch

            #epoch被传入lr_scheduler_func函数中，根据函数返回学习率，赋值给optimizer里的学习率
            set_optimizer_lr(optimizer, lr_scheduler_func, epoch)   #能否输出lr随着epoch变化的图像？看get_lr_table.py文件
