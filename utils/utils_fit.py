...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, save_period, save_dir):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            if iteration >= epoch_step: #训练完了一个epoch（训练时epoch会被定义为：样本数/bs）
                break
            ...
            loss_value = loss_value_all #统计了一个batch size的图片在三个特征层上的总损失，作为这个batch的总损失。

            #----------------------#
            #   反向传播
            #----------------------#
            loss_value.backward() #每一个batch都进行反向传播
            optimizer.step()

            loss += loss_value.item() #每个batch的损失都进行累加，累加到最后就是一个epoch的总损失
